...
Markdown Cleanup is implemented as a separate optional normalizer that runs after Replacement Rules and before Emoji Normalizer, Date Normalizer, and Number Normalizer. It is disabled by default, its options section is collapsed by default, and each cleanup behavior has its own option so users can keep Markdown syntax that their Target TTS Entity already handles well.
